...
public class SlidingViewAbove extends ViewGroup {

	private static final String TAG = "SlidingViewAbove";
	...
	private SlidingViewBehind mViewBehind;
	//	private int mMode;
	...
	/**
	 * Callback interface for responding to changing state of the selected uibase.
	 */
	public interface OnPageChangeListener {

		/**
		 * This method will be invoked when the current uibase is scrolled, either as part
		 * of a programmatically initiated smooth scroll or a user initiated touch scroll.
		 *
		 * @param position Position index of the first uibase currently being displayed.
		 *                 Page position+1 will be visible if positionOffset is nonzero.
		 * @param positionOffset Value from [0, 1) indicating the offset from the uibase at position.
		 * @param positionOffsetPixels Value in pixels indicating the offset from position.
		 */
		public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels);

		/**
		 * This method will be invoked when a new uibase becomes selected. Animation is not
		 * necessarily complete.
		 *
		 * @param position Position index of the new selected uibase.
		 */
	...
	public SlidingViewAbove(Context context) {
		this(context, null);
	}

	public SlidingViewAbove(Context context, AttributeSet attrs) {
		super(context, attrs);
		initCustomViewAbove();
	}
	...
	/**
	 * Set the currently selected uibase. If the CustomViewPager has already been through its first
	...
	/**
	 * Set the currently selected uibase.
	 *
	 * @param item Item index to select
	...
	/**
	 * Set a listener that will be invoked whenever the uibase changes or is incrementally
	...
	// We want the duration of the uibase snap animation to be influenced by the distance that
	...
	public void setCustomViewBehind(SlidingViewBehind cvb) {
		mViewBehind = cvb;
	}
	 ...
	 * @param position Position index of the first uibase currently being displayed.
	 *                 Page position+1 will be visible if positionOffset is nonzero.
	 * @param offset Value from [0, 1) indicating the offset from the uibase at position.
	...
	public boolean arrowScroll(int direction) {
		...
		if (nextFocused != null && nextFocused != currentFocused) {
			if (direction == View.FOCUS_LEFT) {
				...
				// jump to the left, then what we really want to do is uibase right.
			...
			// Trying to move left and nothing there; try to uibase.
			handled = pageLeft();
		} else if (direction == FOCUS_RIGHT || direction == FOCUS_FORWARD) {
			// Trying to move right and nothing there; try to uibase.
			handled = pageRight();
		}
		if (handled) {
